...
static void demoNotif( rfalNfcState st )
{
    ...
    {
        SYS_CONSOLE_MESSAGE("Wake Up mode started \r\n");
    }
    else if( st == RFAL_NFC_STATE_POLL_TECHDETECT )
    {
        if( discParam.wakeupEnabled )
        {
            SYS_CONSOLE_MESSAGE("Wake Up mode terminated. Polling for devices \r\n");
        }
    }
    else if( st == RFAL_NFC_STATE_POLL_SELECT )
    {
        ...
        if( (!multiSel) )
        {
            ...
            SYS_CONSOLE_PRINT("Multiple Tags detected: %d \r\n", devCnt);
        }
        else
        {
...
void wakeup_toggle()
{
    ...
    SYS_CONSOLE_PRINT("Toggling Wake Up mode %s\r\n", discParam.wakeupEnabled ? "ON": "OFF");
...
//                                       ((nfcDevice->type == RFAL_NFC_POLL_TYPE_NFCA) ? PLATFORM_LED_A_PIN  : PLATFORM_LED_F_PIN)  );
                        SYS_CONSOLE_PRINT("Activated in CE %s mode.\r\n", (nfcDevice->type == RFAL_NFC_POLL_TYPE_NFCA) ? "NFC-A" : "NFC-F");

                        if( nfcDevice->rfInterface == RFAL_NFC_INTERFACE_NFCDEP )
                        {
                            SYS_CONSOLE_MESSAGE("Executing Demo P2P\r\n");
                            demoP2P( nfcDevice );
                        }
                        else
                        {
                            SYS_CONSOLE_MESSAGE("Executing Demo CE\r\n");
                            demoCE( nfcDevice );
                        }
                        break;
...
void demoP2P( rfalNfcDevice *nfcDev )
{
    ...
    SYS_CONSOLE_MESSAGE(" Initialize device .. ");
    err = demoTransceiveBlocking( ndefInit, sizeof(ndefInit), &rxData, &rxLen, RFAL_FWT_NONE);
    if( err != ERR_NONE )
    {
        SYS_CONSOLE_MESSAGE("failed.\r\n");
        return;
    }
    SYS_CONSOLE_MESSAGE("succeeded.\r\n");

    SYS_CONSOLE_MESSAGE(" Push NDEF Uri: www.st.com .. ");
    ...
    SYS_CONSOLE_MESSAGE(" Device present, maintaining connection ");
    while(err == ERR_NONE) 
    {
...
void demoAPDU( void )
{
    ...
    if( (err == ERR_NONE) && (rxData[0] == 0x90U) && (rxData[1] == 0x00U) )
    {
        ...
        SYS_CONSOLE_PRINT(" Select CC: %s Data: %s\r\n", (err != ERR_NONE) ? "FAIL": "OK", (err != ERR_NONE) ? "" : hex2Str( rxData, *rxLen) );
        ...
        SYS_CONSOLE_PRINT(" Read CC: %s Data: %s\r\n", (err != ERR_NONE) ? "FAIL": "OK", (err != ERR_NONE) ? "" : hex2Str( rxData, *rxLen) );
    }
#endif /* RFAL_FEATURE_ISO_DEP_POLL */
}
