...
#define IRQ_ST25R_EXTI_IRQn      EIC_IRQn //EXTI0_IRQn
...
#define platformProtectST25RIrqStatus()           platformProtectST25RComm() //OSAL_MUTEX_Lock(&irq_mutex,OSAL_WAIT_FOREVER); //platformProtectST25RComm()                /*!< Protect unique access to IRQ status var - IRQ disable on single thread environment (MCU) ; Mutex lock on a multi thread environment */
#define platformUnprotectST25RIrqStatus()         platformUnprotectST25RComm()  //OSAL_MUTEX_Unlock(&irq_mutex); //platformUnprotectST25RComm()              /*!< Unprotect the IRQ status var - IRQ enable on a single thread environment (MCU) ; Mutex unlock on a multi thread environment         */
...
//#define platformIrqST25RSetCallback( cb )  //we have implemented EIC
...
//#define platformTimerCreate( t )                      SYS_TIME_TimerCreate(0,SYS_TIME_MSToCount(t),&Timer_Callback,(uintptr_t)NULL,SYS_TIME_SINGLE); //timerCalculateTimer(t)                        /*!< Create a timer with the given time (ms)     */
#define platformTimerIsExpired( timer )               SYS_TIME_TimerPeriodHasExpired( timer ) //timerIsExpired(timer)                         /*!< Checks if the given timer is expired        */
#define platformTimerDestroy( timer )                 SYS_TIME_TimerDestroy(timer) //                                              /*!< Stop and release the given timer            */
#define platformDelay( t )                            vTaskDelay(t / portTICK_PERIOD_MS); //HAL_Delay( t )                                /*!< Performs a delay for the given time (ms)    */

//#define platformGetSysTick()                          SYS_TIME_TimerCounterGet(timer, &count) // BSP_GetTick()                                 /*!< Get System Tick ( 1 tick = 1 ms)            */

//#define platformErrorHandle()                         _Error_Handler(__FILE__,__LINE__)             /*!< Global error handler or trap                */

#define platformSpiSelect()                           CLICK_NFC_CS_Clear(); //platformGpioClear(ST25R_SS_PORT, ST25R_SS_PIN)/*!< SPI SS\CS: Chip|Slave Select                */
#define platformSpiDeselect()                         CLICK_NFC_CS_Set();   //platformGpioSet(ST25R_SS_PORT, ST25R_SS_PIN)  /*!< SPI SS\CS: Chip|Slave Deselect              */
...
//#define platformLog(...)                              SYS_CONSOLE_MESSAGE(__VA_ARGS__)                         /*!< Log  method                                 */
